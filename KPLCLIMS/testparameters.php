...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['add'])) {
        ...
        $stmt->bind_param("sssi", $parameterName, $unit, $toleranceLimits, $recommendedCalibrationFrequency);
        ...
        $updateSql = "UPDATE TestParameters 
                      SET parameter_name=?, unit=?, tolerance_limits=?, recommended_calibration_frequency=?
                      WHERE id=?";
        $stmt = $conn->prepare($updateSql);
        $stmt->bind_param("sssii", $parameterName, $unit, $toleranceLimits, $recommendedCalibrationFrequency, $id);
        ...
        $deleteSql = "DELETE FROM TestParameters WHERE id=?";
        ...
        if ($_FILES['file']['error'] == UPLOAD_ERR_OK && $_FILES['file']['size'] > 0) {
            ...
            // Assuming CSV format, you can adjust based on your needs
            $data = array_map('str_getcsv', file($file));
            $header = array_shift($data); // assuming the first row contains column headers
            
            foreach ($data as $row) {
                // Adjust indices based on your CSV structure
                $parameterName = $row[0];
                $unit = $row[1];
                $toleranceLimits = $row[2];
                $recommendedCalibrationFrequency = $row[3];
                
                // Perform the insert into the database (use prepared statements for security)
                $insertSql = "INSERT INTO TestParameters (parameter_name, unit, tolerance_limits, recommended_calibration_frequency) 
                              VALUES (?, ?, ?, ?)";
                $stmt = $conn->prepare($insertSql);
                $stmt->bind_param("sssi", $parameterName, $unit, $toleranceLimits, $recommendedCalibrationFrequency);
...
<form method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
    ...
    <button type="submit" name="upload">Upload from File</button>
</form>
